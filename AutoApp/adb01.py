...
class AdbTools(object):
    ...
    def install(self, path):
        ...
        print('Installing...')
        i = self.adb('install -r %s' % (path,)).read()
        if 'Success' in i:
            print('Install Success')
        if 'Failure' in i:
            reg = re.compile('\\[(.+?)\\]')
            key = re.findall(reg, i)[0]
            try:
    ...
    def uninstall(self, package):
        ...
        """
        print('Uninstalling...')
        i = self.adb('uninstall %s' % (package,)).read()
        print(i)
    ...
    def get_device_ip_address(self):
        ...
        if not self.get_wifi_state() and not self.get_data_state():
            return
        ge = self.shell('ip addr | %s global' % self.__find).read()
        reg = re.compile(r'\d +\.\d+\.\d+\.\d+')
        return re.findall(reg, ge)[0]
    ...
    def get_device_imei(self):
        ...
        # Android 5.0以下方法
        if int(sdk) < 21:
            ge = self.shell('dumpsys iphonesubinfo').read()
            reg = re.compile('[0-9]{15}')
            return re.findall(reg, ge)[0]
        elif self.root():
            ge = self.shell('service call iphonesubinfo 1').read()
            print(ge)
            print(re.findall(re.compile("'.+?'"), ge))
            imei = ''
            for i in re.findall(re.compile("'.+?'"), ge):
    ...
    def get_display_state(self):
        ...
        :return: 亮屏/灭屏
        """
        ge = self.shell('dumpsys power').readlines()
        for i in ge:
            if 'mScreenOn=' in i:
    ...
    def get_screen_reality_size(self):
        ...
        x = 0
        y = 0
        ge = self.shell(r'getevent -p | %s -e "0"' % self.__find).readlines()
        for n in ge:
            if len(n.split()) > 0:
                if n.split()[0] == '0035':
    ...
    def get_wifi_password_list(self):
        ...
        if not self.root():
            print('The device not root.')
            return []
        ge = re.findall(re.compile('ssid=".+?"\\s{3}psk=".+?"'), self.shell('su -c cat /data/misc/wifi/*.conf').read())
        return [re.findall(re.compile('".+?"'), i) for i in ge]
